...
标准差/均方差：$\sqrt{ DX }$记作$\sigma(X)$，
$X$的标准化随机变量：$X^{*}= \frac{X-EX}{\sqrt{ DX }}$，则$EX^{*}=0,DX^{*}=1$
>实际上对数据进行了标准化，将其标准化为期望为0，方差为1的数据

- $DX\geq 0,E(X^{2})=DX+(EX)^{2}\geq (EX)^{2}$
- $Dc=0(c为常数)$
- $D(aX+b)=a^{2}DX$
- $D(X±Y)=DX+DY±2Cov(X,Y)$
- $D\left( \sum_{i=1}^{n} a_{i}X_{i} \right)=\sum_{i=1}^{n}a_{i}^{2}DX_{i}+2\sum_{1\leq i<j\leq n}a_{i}a_{j}Cov(X_{i},X_{j})$
- 若$X$与$Y$相互独立，则$D(aX+bY)=a^{2}DX+b^{2}DY$
- 若$X$与$Y$相互独立，则$D(XY)=DXDY+DX(EY)^{2}+DY(EX)^{2}\geq DXDY$,乘积的方差大于方差的乘积
- $DX\leq E[(X-c)^{2}]$
>右侧看成函数，求一阶导和二阶导可知，$c=EX$时，函数取得极小值，因此该期望大于等于方差
...
$$
\int_{-\infty}^{\infty}e^{- \frac{t^{2}}{2\sigma^{2}}}dt=\sqrt{ 2\pi }\sigma
$$
泊松积分：
...
- 离散型，$E(g(X,Y))=\sum_{j=1}^{\infty}\sum_{i=1}^{\infty}g(x_i,y_i)\cdot p_{ij}$绝对收敛
- 连续性，$E(g(X,Y))=\int_{-\infty}^{+\infty}g(x,y)\cdot f(x,y)$绝对收敛
...
当$DX$越小，$P(|X-\mu|<\epsilon)$越大，表明方差是刻画随机变量与期望值分离程度的量
...
- $|\rho_{XY}|=0$，表示$X,Y$不线性相关，不代表独立
> 线性无关是向量空间里面向量之间的关系
> 独立一定不线性相关，不线性相关不一定独立，可能还有非线性的一些依赖

$Cov(X^{*},Y^{*})=\frac{Cov(X-EX,Y-EY)}{\sqrt{ DX }\sqrt{ DY }}=\rho_{XY}$
#### 独立性与不相关性
