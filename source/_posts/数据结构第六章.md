...
**图**：由顶点集$V$和边集$E$组成，记为$G=(V,E)$，$V(G)$表示图G中顶点的有限非空集合；$E(G)$表示图G中顶点之间的关系（边）集合，$|V|$表示顶点个数，$|G|$表示边的条数。
> 图不可以是空，即$V$一定是非空集
...
**简单路径/回路**：在路径序列中，顶点不重复出现的路径称为**简单路径**。除第一个顶点和最后一个顶点外，其余顶点不重复出现的回路称为**简单回路**。

**距离**：从顶点$u$到$v$的最短路径的长度称为从$u$到$v$的距离。若不存在路径，则距离为无穷大。
...
**强连通图**：在有向图中，若有一对顶点$v$和$w$，从$v$到$w$和从$w$到$v$之间都有路径，则称这两个顶点是强连通的。若图中任意一对顶点都是强连通的，则称此图为强连通图。
...
- 对于无向图，邻接矩阵第$i$行/列的非零元素个数为顶点$i$的度$TD(v_i)$
- 对于有向图，邻接矩阵第i行的非零元素个数为顶点i的出度$OD(v_{i})$；邻接矩阵第i列的非零元素个数为顶点i的入度$ID(v_{i})$
>边数比较多的图更适合采用邻接矩阵存储
### 邻接表法

当图为稀疏图时，使用邻接矩阵法显然会浪费大量存储空间。而邻接表法结合了顺序存储与链式存储的优点，能显著减少这种不必要的空间开销。

对图G的每个顶点$v_i$建立一个单链表，第$i$个单链表的结点表示依附于顶点$v_i$边（对于有向图则是以顶点$v_i$为尾的弧），该单链表就称为顶点$v_i$的边表(对于有向图则称为出边表)。边表的头指针和顶点的数据信息采用顺序存储，称为顶点表
![顶点表和边表结点结构](../images/图/顶点表和边表结点结构.png)
`data`存储顶点$v_i$的相关信息，边表头指针域指向第一条边的边表结点；边表的邻接点域存储与头结点顶点$v_i$邻接的顶点编号,指针域指向下一条边的边表结点。
![图邻接表实例](../images/图/图邻接表实例.png)

邻接表存储结构定义：

一个数组存着该图的所有结点，每一个元素都为一个数组，存着以该结点为头的顶点表结点
...
是有向图的一种链式存储结构，每一条弧用一个弧结点表示，每个顶点用一个顶点结点表示
![弧结点和顶点结点结构](../images/图/弧结点和顶点结点结构.png)

`tailvex`与`headvex`用于存放弧尾结点与弧头结点的的编号，即说明这个是那两个结点之间的弧，`hlink`表示弧头相同的下一条弧，`tlink`表示弧尾相同的下一条弧，`info`权值,`firstin`指向以该点为弧头的第一个弧，`fistout`指向以该点为弧尾的第一条弧
...
`ivex`和`jvex`表示存放该边两头的顶点编号，`ilink`指向依附与顶点`ivex`的下一条边，`jlink`指向依附于`jvex`的下一条边

| 顶点结点   |             |
| ------ | ----------- |
| `data` | `firstedge` |
`data`存放该顶点的相关信息，`firstedge`指向依附于该顶点的第一条边
### 图的基本操作
...
在原有`BFS`的基础上，新增一个数组记录权值，当第一次遍历到或者权值小于原记录的权值时，将新权值与旧权值进行比较取`min`，新全值为当前顶点表+连到这一结点边的权值
...
给定一个源点，求解 从源点到其余各个顶点的最短路径长度的单源最短路径算法。（边权值不为负数）
...
2. 准备小根堆的顶点集，存放（x点，源点到x距离）
3. 令`distance[源点]=0`，其余都尾`Max`，顶点元素`(源点，0)`进入小根堆
4. 从小根堆弹出(u点，源点到u的距离)
5. 若`visited[u]==true`,不做任何处理，重复步骤4
> 意味着这个顶点相关的边已经进行处理过了
6. 若`visited[u]==false`，令`visited[u]==true`,然后观察u的每一条边，设某边去往`v`,权值 为`w`
7. 若`distance[u]+w<distance[v]`，令`distance[v]=distance[u]+w`,把顶点`v`入堆，即`(v,distance[u]+w)`加入小根堆
> 为什么要将顶点`v`加入堆中：
> 1. 原本距离是$\infty$,加入堆代表当前图能够连到`v`点，需要用`v`对当前路径进行优化
> 2. 已经有明确的距离了，仅作加入，用于同一判断更小距离后的步骤，弹出时不做处理
8. 处理完u的每一条 边后 ，重复步骤4
...
while (堆不为空) {
    ...
    visited[u] = true;  // 标记为已确定最短路
    
    // 松弛操作：检查u的所有邻居
    for (每条边 u→v，权重w) {
        if (distance[u] + w < distance[v]) {
            distance[v] = distance[u] + w;堆.push((v, distance[v]));  // 注意：同一个顶点可能多次入堆！
        }
    }
}
```
...
同样的，也可以参照prim算法，构建小根堆的边集，在当前边集里找出最短的边集来处理，小的边挨个弹出，判断和原数值的大小
>最短路径必然是min(直连，最小边之和)
...
跳板在最外层循环：针对所有的i和j之间的距离，都要尝试所有的跳板
...
对具有明确先后顺序或前置任务来制定任务执行的具体顺序

在带权有向图中，若以顶点表示事件，以有向边表示活动，并以边上的权值表示完成该活动所需的开销(如时间)，则称这种图为用边表示活动的网络，简称`AOE`网。

`AOE`网和`AOV`网同属有向无环图，但二者在定义上有本质区别:`AOE` 网的边带有权值，代表活动及其持续时间，而顶点表示事件;`AOV`网的边无权值，仅用于表示顶点(活动)之间的前后关系。
...
在带权有向图中，以顶点表示事件，以有向边表示活动，以边上的权值表示完成该活动的开销（如完成活动所需的时间），称为用边表示活动的网络，简称AOE网
...
在AOE网中仅有一个入读为0的顶点，为开始顶点（源点），也仅有一个出度为0的顶点，为结束顶点（汇点）。从源点到汇点的所有路径中，具有最大路径长度的路径称为**关键路径**，关键路径上的活动称为关键活动。

**事件$v_{k}$的最早发生事件$ve(k)$**

**活动$a_{i}$的最早开始时间**：指的是这个活动连接的节点的时间

**活动$a_{i}$的最晚开始时间**：指的是这个活动连接的下一个节点最晚开始时间-当前活动耗时

**时间余量**：活动可拖延的时间，最晚时间-最早时间，时间余量为0的活动就是关键活动
