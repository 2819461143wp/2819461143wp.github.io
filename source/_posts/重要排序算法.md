...
public static int[] help = new int[MAXN];//辅助数组，在辅助数组里有序
//T(n)=2*T(n/2)+O(n)
...
反向来实现有序，从两个数有序，到四个数有序依次往上实现整个数组有序
...
public static int partition1(int l, int r, int x){
    int a = l, xi =0;//a表示小于x的数组的右边界
    for (int i = l; i <= r; i++){
        if(arr[i] <= x){
            swap(arr[a], arr[i]);//交换两个索引的值
            if (arr[a] == x){
                xi = a;//记录最后一个等于划分值的索引
            }
            a++;
    ...
    //l,....,x,.....a-1,a,...
    //l~a-1 <=x   a~r >x
    swap(arr[xi], arr[a - 1]);//将该值换到中间a-1去
    return a - 1;
}
```
...
相较于经典过程，将相同的比较值`x`都放到了中间，因此需要一个左侧数组的右边界和右侧数组的左边界
